...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO tblcustompedals (id, cpname, cppic, cppicsmall, cpprice, cpdesc, cp_cat_id, cplongdesc, picalttag) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
mysql_select_db($database_deanelectro, $deanelectro);
$query_rstnewcp = "SELECT * FROM tblcustompedals";
$rstnewcp = mysql_query($query_rstnewcp, $deanelectro) or die(mysql_error());
$row_rstnewcp = mysql_fetch_assoc($rstnewcp);
$totalRows_rstnewcp = mysql_num_rows($rstnewcp);

mysql_select_db($database_deanelectro, $deanelectro);
$query_rstnewcpcats = "SELECT * FROM otblcats";
$rstnewcpcats = mysql_query($query_rstnewcpcats, $deanelectro) or die(mysql_error());
$row_rstnewcpcats = mysql_fetch_assoc($rstnewcpcats);
$totalRows_rstnewcpcats = mysql_num_rows($rstnewcpcats);
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>Dean Electronics - Custom Pedals - New Product</title>
    ...
    <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
      <table align="center">
        <tr valign="baseline">
          ...
          <td><input type="text" name="cpname" value="" size="32" /></td>
        </tr>
        <tr valign="baseline">
          <td nowrap="nowrap" align="right">Main Image URL:</td>
          <td><input type="text" name="cppic" value="" size="32" /></td>
        </tr>
        <tr valign="baseline">
          <td nowrap="nowrap" align="right">Small Image URL</td>
          <td><input type="text" name="cppicsmall" value="" size="32" /></td>
        </tr>
        <tr valign="baseline">
          <td nowrap="nowrap" align="right">Price:</td>
          <td><input type="text" name="cpprice" value="" size="32" /></td>
        </tr>
        <tr valign="baseline">
          <td nowrap="nowrap" align="right">Short Description:</td>
          <td><input type="text" name="cpdesc" value="" size="32" /></td>
        </tr>
        <tr valign="baseline">
...
do {  
?>
            <option value="<?php echo $row_rstnewcpcats['catid']?>" ><?php echo $row_rstnewcpcats['catname']?></option>
            <?php
} while ($row_rstnewcpcats = mysql_fetch_assoc($rstnewcpcats));
?>
          </select></td>
        </tr>
        ...
        <tr valign="baseline">
          ...
          <td><input type="text" name="picalttag" value="" size="32" /></td>
        </tr>
        <tr valign="baseline">
          <td nowrap="nowrap" align="right" valign="top">Long Description:</td>
          <td><textarea name="cplongdesc" id="cplongdesc" cols="45" rows="5"></textarea></td>
        </tr>
...
<?php
mysql_free_result($rstnewcp);

mysql_free_result($rstnewcpcats);
?>
